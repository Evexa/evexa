...
$sTableID = "b_sotbit_reviews_questions";
...
if ($lAdmin->EditAction()) {
    foreach ($FIELDS as $ID => $arFields) {
        ...
        if ($ID > 0) {
            // Get old fields
            $arFieldOld = QuestionsTable::getById($ID);
            ...
            $result = QuestionsTable::update($ID, $arData);
            unset($arData);
            if (!$result->isSuccess()) {
                $lAdmin->AddGroupError(
                    Loc::getMessage("LIST_QUESTIONS_ELEMENT_SAVE_ERROR") . " "
                    . Loc::getMessage("LIST_QUESTIONS_ELEMENT_NO_ZAPIS"),
                    $ID
                );
            } else {
    ...
    foreach ($arID as $ID) {
        ...
        switch ($_REQUEST['action']) {
            case "delete" :
                ...
                if (!$result->isSuccess()) {
                    $lAdmin->AddGroupError(
                        Loc::getMessage("LIST_QUESTIONS_ELEMENT_DEL_ERROR") . " "
                        . Loc::getMessage("LIST_QUESTIONS_ELEMENT_NO_ZAPIS"),
                        $ID
                    );
                } else {
                    $lAdmin->AddActionSuccessMessage(Loc::getMessage('LIST_QUESTIONS_ELEMENT_DEL_SUCCESS'));
...
$rsData->NavStart();
$lAdmin->NavText($rsData->GetNavPrint(Loc::getMessage("LIST_QUESTIONS_ELEMENT_NAV")));
$lAdmin->AddHeaders([
    [
        "id" => "ID",
        "content" => Loc::getMessage("LIST_QUESTIONS_ELEMENT_TABLE_ID"),
        "sort" => "ID",
        "align" => "right",
        "default" => true
    ],
    [
        "id" => "ID_ELEMENT",
        "content" => Loc::getMessage("LIST_QUESTIONS_ELEMENT_TABLE_ID_ELEMENT"),
        "sort" => "ID_ELEMENT",
        "default" => true
    ],
    [
        "id" => "XML_ID_ELEMENT",
        "content" => Loc::getMessage("LIST_QUESTIONS_ELEMENT_TABLE_XML_ID_ELEMENT"),
        "sort" => "XML_ID_ELEMENT",
        "default" => true
    ],
    [
        "id" => "ID_USER",
        "content" => Loc::getMessage("LIST_QUESTIONS_ELEMENT_TABLE_ID_USER"),
        "sort" => "ID_USER",
        "align" => "right",
        "default" => true
    ],
    [
        "id" => "DATE_CREATION",
        "content" => Loc::getMessage("LIST_QUESTIONS_ELEMENT_TABLE_DATE_CREATION"),
        "sort" => "DATE_CREATION",
        "default" => true
    ],
    [
        "id" => "QUESTION",
        "content" => Loc::getMessage("LIST_QUESTIONS_ELEMENT_TABLE_QUESTION"),
        "sort" => "QUESTION",
        "default" => true
    ],
    [
        "id" => "MODERATED",
        "content" => Loc::getMessage("LIST_QUESTIONS_ELEMENT_TABLE_MODERATED"),
        "sort" => "MODERATED",
        "default" => true
    ],
    [
        "id" => "ACTIVE",
        "content" => Loc::getMessage("LIST_QUESTIONS_ELEMENT_TABLE_ACTIVE"),
        "sort" => "ACTIVE",
        "default" => true
    ]
]);
...
while ($arRes = $rsData->NavNext(true, "f_")) {
    ...
    if ($el_arr = $el_res->GetNext()) {
        $row->AddViewField(
            "ID_ELEMENT",
            '<a href="sotbit.reviews_questions_edit.php?ID=' . $f_ID . '&lang=' . LANG . '"">[' . $el_arr['ID'] . '] ' . $el_arr['NAME'] . '</a>'
        );
    }
    unset($el_res);
    unset($el_arr);
    ...
    if ($arItem = $Users->Fetch()) {
        ...
        $row->AddViewField("ID_USER", Loc::getMessage('LIST_QUESTIONS_ELEMENT_NO_AUTH_USER'));
    }
    unset($f_ID_USER);
    unset($Users);
    unset($arItem);
    ...
    $row->AddViewField("QUESTION", $f_QUESTION);
    unset($f_QUESTION);
    $row->AddCheckField(
        "MODERATED",
        ($f_MODERATED == 'Y')
            ? Loc::getMessage('LIST_QUESTIONS_ELEMENT_POST_YES')
            : Loc::getMessage('LIST_QUESTIONS_ELEMENT_POST_NO')
    );
    ...
    $arActions[] = [
        "ICON" => "edit",
        "DEFAULT" => true,
        "TEXT" => Loc::getMessage("LIST_QUESTIONS_ELEMENT_EDIT"),
        "ACTION" => $lAdmin->ActionRedirect("sotbit.reviews_questions_edit.php?ID=" . $f_ID)
    ];

    if ($POST_RIGHT >= "W") {
        $arActions[] = [
            "ICON" => "delete",
            "TEXT" => Loc::getMessage("LIST_QUESTIONS_ELEMENT_DEL"),
            "ACTION" => "if(confirm('" . Loc::getMessage('LIST_QUESTIONS_ELEMENT_DEL_CONF') . "')) " . $lAdmin->ActionDoGroup($f_ID,
                    "delete")
        ];
    }
    $arActions[] = [
...
$lAdmin->AddFooter([
    [
        "title" => Loc::getMessage("LIST_QUESTIONS_ELEMENT_LIST_SELECTED"),
        "value" => $rsData->SelectedRowsCount()
    ],
    [
        "counter" => true,
        "title" => Loc::getMessage("LIST_QUESTIONS_ELEMENT_LIST_CHECKED"),
        "value" => "0"
    ]
]);
$Moderation = [
    "delete" => Loc::getMessage("LIST_QUESTIONS_ELEMENT_LIST_DELETE"),
    "activate" => Loc::getMessage("LIST_QUESTIONS_ELEMENT_LIST_ACTIVATE"),
    "deactivate" => Loc::getMessage("LIST_QUESTIONS_ELEMENT_LIST_DEACTIVATE")
];
$Moderation = array_merge(
    $Moderation,
    [
        "moderate" => Loc::getMessage("LIST_QUESTIONS_ELEMENT_LIST_MODERATE"),
        "unmoderate" => Loc::getMessage("LIST_QUESTIONS_ELEMENT_LIST_UNMODERATE")
    ]
);
...
$APPLICATION->SetTitle(Loc::getMessage("LIST_QUESTIONS_ELEMENT_TITLE"));
require($_SERVER["DOCUMENT_ROOT"] . "/bitrix/modules/main/include/prolog_admin_after.php");
$Moderation = [
    Loc::getMessage("LIST_QUESTIONS_ELEMENT_ID"),
    Loc::getMessage("LIST_QUESTIONS_ELEMENT_ID_ELEMENT"),
    Loc::getMessage("LIST_QUESTIONS_ELEMENT_XML_ID_ELEMENT"),
    Loc::getMessage("LIST_QUESTIONS_ELEMENT_ID_USER"),
    Loc::getMessage("LIST_QUESTIONS_ELEMENT_QUESTION"),
    Loc::getMessage("LIST_QUESTIONS_ELEMENT_MODERATED"),
    Loc::getMessage("LIST_QUESTIONS_ELEMENT_ACTIVE")
];
...
?>
    <form name="find_form" method="get"
        ...
        <? $oFilter->Begin(); ?>
        <tr>
            <td><b><?= Loc::getMessage("LIST_QUESTIONS_ELEMENT_FIND") ?>:</b></td>
            <td><input type="text" size="25" name="find"
                       value="<?= htmlspecialchars($find) ?>"
                       title="<?= Loc::getMessage("LIST_QUESTIONS_ELEMENT_FIND_TITLE") ?>">
                <?
                $arr = [
                    "reference" => [
                        "ID"
        ...
        <tr>
            <td><?= Loc::getMessage("LIST_QUESTIONS_ELEMENT_ID") ?>:</td>
        ...
        <tr>
            <td><?= Loc::getMessage("LIST_QUESTIONS_ELEMENT_ID_ELEMENT") ?>:</td>
        ...
        <tr>
            <td><?= Loc::getMessage("LIST_QUESTIONS_ELEMENT_XML_ID_ELEMENT") ?>:</td>
        ...
        <tr>
            <td><?= Loc::getMessage("LIST_QUESTIONS_ELEMENT_ID_USER") ?>:</td>
        ...
        <tr>
            <td><?= Loc::getMessage("LIST_QUESTIONS_ELEMENT_QUESTION") ?>:</td>
        ...
        <tr>
            <td><?= Loc::getMessage("LIST_QUESTIONS_ELEMENT_MODERATED") ?>:</td>
            <td>
                <?
                $arr = [
                    "reference" => [
        ...
        <tr>
            <td><?= Loc::getMessage("LIST_QUESTIONS_ELEMENT_ACTIVE") ?>:</td>
            <td>
                <?
                $arr = [
                    "reference" => [
